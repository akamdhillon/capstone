...
class Settings:
    # Network — set JETSON_IP and BACKEND_IP in .env
    JETSON_IP = os.getenv("JETSON_IP")
    BACKEND_IP = os.getenv("BACKEND_IP")
    BACKEND_BASE_URL = f"http://{os.getenv('BACKEND_IP')}:8000" if os.getenv("BACKEND_IP") else None
    ...
    # LLM — OLLAMA_HOST: localhost for Mac, or http://JETSON_IP:11434 when Ollama runs on Jetson
